...
class BenchmarkEvaluator:
    def __init__(
        ...
        self.workspace_root = Path(workspace_root) if workspace_root is not None else Path(
            tempfile.mkdtemp(prefix="niko_agent-benchmark-")
        )
    ...
    def run_task(self, task):
        ...
        session_store = SessionStore(fixture_copy_root / ".niko" / "sessions")
        run_store = RunStore(fixture_copy_root / ".niko" / "runs")
        ...
        verifier_command = str(task["verifier"])
        if os.name == "nt" and verifier_command.startswith("python3 "):
            verifier_command = (
                subprocess.list2cmdline([sys.executable]) + verifier_command[len("python3") :]
            )
