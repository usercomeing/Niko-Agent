"""Read-only helpers for Niko run/session evidence.

These helpers intentionally do not import or instantiate the Niko runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.niko/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".niko" / "runs")
        ...
        session_path = _latest_file(workspace / ".niko" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".niko" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
