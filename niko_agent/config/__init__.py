...
DEFAULT_CONFIG_PATH = Path.home() / ".config" / "niko" / "config.toml"
PROJECT_CONFIG_NAME = ".niko.toml"
...
ENV_PROVIDER = "NIKO_PROVIDER"
ENV_API_KEY = "NIKO_API_KEY"
ENV_BASE_URL = "NIKO_BASE_URL"
ENV_MODEL = "NIKO_MODEL"
...
LEGACY_ENV_NAMES = {
    "openai": {
        "api_key": ("NIKO_OPENAI_API_KEY", "OPENAI_API_KEY"),
        "base_url": ("NIKO_OPENAI_API_BASE", "OPENAI_API_BASE", "OPENAI_BASE_URL"),
        "model": ("NIKO_OPENAI_MODEL", "OPENAI_MODEL"),
    },
    "anthropic": {
        "api_key": (
        ...
        "base_url": (
            "NIKO_ANTHROPIC_API_BASE",
            "ANTHROPIC_API_BASE",
            "ANTHROPIC_BASE_URL",
        ),
        "model": ("NIKO_ANTHROPIC_MODEL", "ANTHROPIC_MODEL"),
    },
    "deepseek": {
        "api_key": ("NIKO_DEEPSEEK_API_KEY", "DEEPSEEK_API_KEY"),
        "base_url": (
            "NIKO_DEEPSEEK_API_BASE",
            "DEEPSEEK_API_BASE",
            "DEEPSEEK_BASE_URL",
        ),
        "model": ("NIKO_DEEPSEEK_MODEL", "DEEPSEEK_MODEL"),
    },
}
...
def resolve_provider_config(
    ...
    *,
    ...
    protocol = _first_value(
        None,
        os.environ.get("NIKO_PROTOCOL"),
        profile_values.get("protocol"),
        legacy_env.get("NIKO_PROTOCOL"),
...
def _read_config_file(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"invalid Niko config file {path}: {exc}") from exc
    except OSError as exc:
        raise ValueError(f"could not read Niko config file {path}: {exc}") from exc
