...
这个模块负责把“用户怎么启动 niko_agent”翻译成 runtime 能理解的对象：
...
DEFAULT_SECRET_ENV_NAMES = (
    ...
    "OPENAI_API_KEY",
    "OPENAI_API_TOKEN",
    "NIKO_ANTHROPIC_API_KEY",
    "ANTHROPIC_API_KEY",
    "ANTHROPIC_AUTH_TOKEN",
    "NIKO_DEEPSEEK_API_KEY",
    "DEEPSEEK_API_KEY",
    "NIKO_RIGHT_CODES_API_KEY",
    "RIGHT_CODES_API_KEY",
    "GITHUB_PAT",
    "GH_PAT",
)
...
SECRET_ENV_NAMES_VAR = "NIKO_SECRET_ENV_NAMES"
...
def build_agent(args):
    ...
    - 输出：一个新的 `Niko`，或一个从旧 session 恢复出来的 `Niko`
    ...
    store = SessionStore(workspace.repo_root + "/.niko/sessions")
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--config", default=None, help="Path to a Niko TOML config file."
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    parser.add_argument(
        "--memory-dir",
        default=None,
        help="Memory directory. Defaults to .niko/memory in the workspace.",
    )
    parser.add_argument(
        "--no-auto-dream",
        action="store_true",
...
def handle_repl_command(agent, user_input):
    ...
    if command_name == "fork":
        try:
            result = agent.fork_session(command_args or "latest")
        except ValueError as exc:
            return True, False, f"error: {exc}"
        return True, False, json.dumps(result, indent=2, sort_keys=True)
    if command_name == "rollback":
        try:
            if command_args.startswith("--confirm "):
                result = agent.confirm_rollback(command_args.split(None, 1)[1])
            else:
                result = agent.preview_rollback(command_args or "latest")
                result = dict(result, confirm=f"/rollback --confirm {result['token']}")
        except ValueError as exc:
            return True, False, f"error: {exc}"
        return True, False, json.dumps(result, indent=2, sort_keys=True)
...
def main(argv=None):
    ...
    if mode == "tui":
        from .tui.app import NikoTuiApp
