...
Niko owns session state, workspace context, memory, checkpoints, and persistence.
...
from .session_branch import (
    confirm_runtime_rollback,
    fork_runtime_session,
    preview_runtime_rollback,
)
...
class Niko(RuntimeSecretsMixin, RuntimeCheckpointsMixin):
    def __init__(
        self,
        model_client,
        workspace,
        ...
        self.run_store = run_store or RunStore(
            Path(workspace.repo_root) / ".niko" / "runs"
        )
    ...
    def build_prefix(self):
        ...
        text = textwrap.dedent(
            f"""\
            You are Niko, a small local coding agent working inside a local repository.
    ...
    def preview_rollback(self, target="latest"):
        return preview_runtime_rollback(self, target)

    def confirm_rollback(self, token):
        return confirm_runtime_rollback(self, token)
