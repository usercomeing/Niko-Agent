...
from .sensitive_paths import SENSITIVE_BASENAMES, SENSITIVE_SUFFIXES
...
SHELL_SENSITIVE_RE = re.compile(
    r"(?:^|[\\/\s'\"])(?:\.niko|\.ssh|\.gnupg|\.env(?:\.[^\s'\"]+)?|"
    + "|".join(re.escape(name) for name in sorted(SENSITIVE_BASENAMES))
    + r"|[^\s'\"]+(?:" + "|".join(re.escape(suffix) for suffix in SENSITIVE_SUFFIXES) + r"))(?:$|[\\/\s'\"])",
    re.IGNORECASE,
)
        ...
        if tool.name == "run_shell":
            ...
            if SHELL_SENSITIVE_RE.search(command):
                return ToolPolicyDecision.deny(
                    "sensitive_path",
                    "error: run_shell cannot access sensitive paths or Niko private state",
                )
