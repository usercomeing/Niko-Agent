...
Niko is a small local coding agent that turns one user request into a bounded
...
- `Niko` owns session state, memory, tools, and workspace safety.
...
be validated through persisted `.niko/sessions/*.json` and
`.niko/sessions/*.events.jsonl` artifacts before treating a runtime change as
done.
...
- `.niko/runs/<run_id>/task_state.json`
- `.niko/runs/<run_id>/trace.jsonl`
- `.niko/runs/<run_id>/report.json`
