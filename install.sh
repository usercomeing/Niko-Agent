...
REPO="${NIKO_REPO:-https://github.com/usercomeing/Niko-Agent.git}"
INSTALL_DIR="${NIKO_INSTALL_DIR:-$HOME/.niko-agent}"
BRANCH="${NIKO_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[niko]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[niko]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[niko]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[niko] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
    printf "\n${BOLD}╔══════════════════════════════════════════╗${RESET}\n"
    printf   "${BOLD}║        Niko-Agent 一键安装                  ║${RESET}\n"
    printf   "${BOLD}╚══════════════════════════════════════════╝${RESET}\n\n"
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    if [[ -d "$INSTALL_DIR/.git" ]]; then
        ...
        info "克隆 Niko-Agent 到 ${INSTALL_DIR} ..."
        rm -rf "$INSTALL_DIR"
    ...
    BIN_DIR="${NIKO_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/niko"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/niko" "\$@"
EOF
    chmod +x "$LAUNCHER"

    printf "\n"
    success "Niko-Agent 安装完成！"
    printf "\n"
