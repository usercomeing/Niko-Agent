...
from niko_agent.evaluation.metrics import (
    _provider_profile,
...
def test_provider_profile_uses_project_toml_before_legacy_niko_agent_env(tmp_path, monkeypatch):
    ...
    with patch.dict(
        os.environ,
        {
            "NIKO_DEEPSEEK_API_KEY": "sk-legacy-deepseek",
            "NIKO_DEEPSEEK_MODEL": "legacy-deepseek-model",
            "NIKO_DEEPSEEK_API_BASE": "https://legacy.deepseek.example/anthropic",
        },
        clear=True,
    ):
...
def test_write_benchmark_core_report_marks_resume_safe_metrics(tmp_path):
    ...
    report_path = tmp_path / "docs" / "metrics" / "niko_agent-benchmark-core-report.md"
